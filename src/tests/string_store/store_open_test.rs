// Tests StringStore open behavior and file creation/error handling.
...
fn open_creates_new_file() {
    let path = temp_file("string_open_creates_new_file");
    assert!(!path.exists());

    let result = StringStore::open(&path);
...
fn open_invalid_path_returns_file_open_error() {
    ...
    let result = StringStore::open(&dir_path);
