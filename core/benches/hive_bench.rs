...
use hive_core::db::hive_db::{HiveDb, Property};
use hive_core::types::NodeId;
use hive_core::value::{self, Value};
...
fn full_scan_lookup(db: &mut HiveDb, target: i64) -> Vec<NodeId> {
    ...
    for node_id in 0..count {
        if db.get_node_property(node_id, "user_id").unwrap().as_ref() == Some(&expected) {
...
fn benchmark_indexed_lookup_vs_scan(c: &mut Criterion) {
    ...
    group.bench_function("indexed_lookup", |b| {
        b.iter(|| {
            black_box(
                indexed_db
                    .lookup_node_ids_by_property("user_id", &Value::Integer(indexed_target))
                    .unwrap(),
            );
        })
    });
