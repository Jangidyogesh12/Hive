...
pub struct IndexStore {
    ...
    edge_property_index: HashMap<PropertyIndexKey, Vec<EdgeId>>,
}
...
impl IndexStore {
    pub fn load_or_rebuild(
        ...
        if index_path.exists()
            && let Ok(indexes) = Self::load(index_path)
            && indexes.validate_posting_lists()
        {
            return Ok(indexes);
        }
    ...
    pub fn load(index_path: &Path) -> Result<Self, DbError> {
        ...
        let edge_property_bucket_count = read_u64(&mut file)?;
        for _ in 0..edge_property_bucket_count {
            let key_hash = read_u64(&mut file)?;
            let value = read_index_value(&mut file)?;
            let posting_len = read_u64(&mut file)?;
            let mut posting = Vec::with_capacity(posting_len as usize);
    ...
    pub fn save(&self, index_path: &Path) -> Result<(), DbError> {
        ...
        write_u64(&mut file, self.edge_property_index.len() as u64)?;
        for (key, edge_ids) in &self.edge_property_index {
            write_u64(&mut file, key.key_hash)?;
            write_index_value(&mut file, &key.value)?;
            write_u64(&mut file, edge_ids.len() as u64)?;
            for edge_id in edge_ids {
    ...
    pub fn rebuild(
        ...
        for edge_id in 0..edge_count {
            ...
            indexes.index_edge_record(edge_id, &edge, property_store, string_store)?;
        }
    ...
    pub fn lookup_edges_by_property(&self, key_hash: u64, value: &Value) -> Vec<EdgeId> {
        let key = PropertyIndexKey {
            key_hash,
            value: Self::normalize_value(value),
        };
    ...
    pub fn remove_node(
        ...
        Self::walk_properties(node.first_property, property_store, string_store, |prop| {
    ...
    pub fn insert_edge(
        ...
        self.index_edge_record(edge_id, edge, property_store, string_store)
    }
    ...
    pub fn remove_edge(
        ...
        self.remove_edge_type(edge_id, edge);

        let mut keys = Vec::new();
        Self::walk_properties(edge.first_property, property_store, string_store, |prop| {
            keys.push(PropertyIndexKey {
                key_hash: prop.key_hash,
                value: prop.value,
            });
        ...
        for key in keys {
            remove_from_index(&mut self.edge_property_index, &key, edge_id);
        }

        Ok(())
    }

    pub fn upsert_edge_property(
        &mut self,
        edge_id: EdgeId,
        key_hash: u64,
        old_value: Option<&Value>,
        ...
        if let Some(old) = old_value {
            ...
            remove_from_index(&mut self.edge_property_index, &old_key, edge_id);
        }
        ...
        append_unique(
            self.edge_property_index.entry(new_key).or_default(),
            edge_id,
        );
    }

    fn insert_edge_type(&mut self, edge_id: EdgeId, edge: &EdgeRecord) {
        append_unique(
    ...
    fn remove_edge_type(&mut self, edge_id: EdgeId, edge: &EdgeRecord) {
    ...
    fn index_edge_record(
        ...
        self.insert_edge_type(edge_id, edge);
        ...
        Self::walk_properties(edge.first_property, property_store, string_store, |prop| {
            ...
            append_unique(self.edge_property_index.entry(key).or_default(), edge_id);
            Ok(())
        })
    }
        ...
        F: FnMut(NormalizedProperty) -> Result<(), DbError>,
    {
        ...
        while prop_id != NIL_ID {
            ...
            if let Some(value) = Self::read_property_value(&prop, string_store)? {
    ...
    fn read_property_value(
        prop: &PropertyRecord,
        string_store: &mut StringStore,
    ) -> Result<Option<IndexValue>, DbError> {
        ...
        Ok(Some(Self::normalize_value(&value)))
    }
...
fn remove_from_index(
    index: &mut HashMap<PropertyIndexKey, Vec<u64>>,
    key: &PropertyIndexKey,
    id: u64,
) {
    if let Some(ids) = index.get_mut(key) {
        remove_id_from_posting(ids, id);
    }
