use crate::query::{
    ...
    parser::parse,
    planner::{NodeIndexHint, QueryPlan, plan},
};
...
fn parse_match_node_with_where_and_alias_return() {
    let stmt = parse(
        "MATCH (n:Person {active: true}) WHERE n.age > 25 RETURN n.name AS person_name, n.age",
    )
    .unwrap();
    ...
    match stmt {
        Statement::Match(clause) => {
            ...
            assert_eq!(
                clause.return_clause.items[0].alias,
...
#[test]
fn planner_emits_label_and_property_index_hint_for_match_scan() {
    let stmt = parse("MATCH (n:Person {name: \"Alice\"}) RETURN n").unwrap();
    let query_plan = plan(stmt).unwrap();

    match query_plan {
        QueryPlan::Sequence(steps) => match &steps[0] {
            QueryPlan::ScanNodes { index_hint, .. } => match index_hint {
                NodeIndexHint::LabelAndProperty { label, key, value } => {
                    assert_eq!(label, "Person");
                    assert_eq!(key, "name");
                    assert_eq!(value, &Value::String("Alice".to_string()));
                }
                _ => panic!("expected label+property index hint"),
            },
...
fn parse_match_where_boolean_expression_tree() {
    let stmt = parse("MATCH (n:Person) WHERE n.age > 18 AND n.active = true OR n.age = 0 RETURN n")
        .unwrap();
