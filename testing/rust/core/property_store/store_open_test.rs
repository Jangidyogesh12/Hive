// Tests PropertyStore open behavior and file creation/error handling.
...
fn open_creates_new_file() {
    let path = temp_file("property_open_creates_new_file");
    assert!(!path.exists());
...
fn open_invalid_path_returns_file_open_error() {
    ...
    let result = PropertyStore::open(&dir_path);
